-- grid_ui.lua -- draws the isomorphic CPS keyboard and handles grid presses.
-- The LEFTMOST column (x=1) is reserved as a "rotation" strip: each row selects
-- an offset along the CPS rotation axis, sliding the visible lattice slice so
-- every note in the set is reachable. The remaining columns are the keyboard.
-- Cells are keyed by frequency (idiom from pitfalls/lib/g.lua); all cells of a
-- held pitch light together. Works with any single grid size.

local grid_ui = {}

local g = grid.connect()
local current_layout = nil
local on_cell, off_cell, on_rotate = nil, nil, nil
local RESERVED = 1              -- leftmost column reserved for rotation
local rot_offset = 0            -- currently selected offset (for drawing)
...
function grid_ui.set_handlers(on_fn, off_fn, rot_fn)
  on_cell, off_cell, on_rotate = on_fn, off_fn, rot_fn
end
...
g.key = function(x, y, z)
  if x <= RESERVED then                        -- rotation strip
...
function grid_ui.redraw(active)
  ...
  for y = 1, h do
    ...
    local level
    if o == rot_offset then level = 15        -- current slice
    elseif o == 0 then level = 6              -- home (offset 0)
    else level = 2 end
    g:led(1, y, level)
  end

  -- keyboard (x = RESERVED+1 .. width)
